...
public class BattleSimulation {
    ...
    public Squad simulateGalacticWar(Squad firstSquad, Squad secondSquad, Squad alfiSquad) {
        ...
        {
            battleLogs.add(new BattleLog("El primer escuadrón ha ganado, la humanidad ha triunfado."));
            return firstSquad;
        } else if (simulateAutoSquadBattle(secondSquad, alfiSquad) == secondSquad) //Checks if the second human squad won
        {   
            battleLogs.add(new BattleLog("El segundo escuadrón ha ganado, la humanidad ha triungado."));
            return secondSquad;
        } else { //If neither of them won... then that means the humans lost and the alfiSquad has won the war
            battleLogs.add(new BattleLog("Ambos escuadrones superhumanos han sido erradicados. La humanidad será exterminada"));
            return alfiSquad;
        }
    ...
    public Squad simulateAutoSquadBattle(Squad humanSquad, Squad alfiSquad) {
        ...
        while (humanIterator.hasNext() && alfiIterator.hasNext()) {
            ...
            fastSoldier = determineFirstAttacker(humanIterator.next(), alfiIterator.next());
            
            slowSoldier = (fastSoldier == humanIterator ? alfiIterator : humanIterator).next();
            ...
            while (checkAttackPhaseEnd(fastSoldier, slowSoldier) == false) {
            ...
            if (humanSquad.getSoldiers().get(0).getHealth() <= 0 || humanSquad.getSoldiers().get(0).getWisdom() <= 0) {
                ...
                battleLogs.add(new BattleLog(humanSquad.getSoldiers().get(0).getName()+ "murió"));
                ...
                battleLogs.add(new BattleLog(alfiSquad.getSoldiers().get(0).getName()+ "murió"));
